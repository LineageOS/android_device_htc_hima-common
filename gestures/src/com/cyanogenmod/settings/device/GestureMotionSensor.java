...
public class GestureMotionSensor {
    ...
    public void enable() {
        ...
        try {
            currentlyEnabled = Integer.parseInt(val, 16);
            toEnable = currentlyEnabled | sensorTypeToSysfsType(mType);
        } catch (NumberFormatException e) {
            Log.w(TAG, "Failed to parse control path, unable to enable sensor");
            return;
        }
        if (!FileUtils.writeLine(CONTROL_PATH, Integer.toHexString(toEnable))) {
            Log.w(TAG, "Failed to write control path, unable to enable sensor");
            return;
        }
