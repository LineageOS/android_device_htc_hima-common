        ...
        int len)
{
    ...
    while (bytes_read < len) {
        memset(&cmd_buf, 0, sizeof(cmd_buf));
        cmd_buf.size = MIN(I2C_BUFFER_SIZE, len - bytes_read);
        if ((ret = ioctl(amp->fd, TFA9895_READ_CONFIG(cmd_buf),
                        &cmd_buf)) != 0) {
            ALOGE("ioctl TFA9895_READ_CONFIG failed, ret = %d", -errno);
            goto read_err;
        ...
        const uint8_t *buf, int len)
{
    ...
    do {
        ...
        memset(&cmd_buf, 0, sizeof(cmd_buf));
        to_write = MIN(len - written, MAX_I2C_LENGTH - 1);
        cmd_buf.size = to_write + 1;
        cmd_buf.buffer[0] = (0xFF & addr);
        memcpy(cmd_buf.buffer + 1, buf + written, to_write);
        if ((ret = ioctl(amp->fd, TFA9895_WRITE_CONFIG(cmd_buf),
                        &cmd_buf)) != 0) {
            ALOGE("ioctl TFA9895_WRITE_CONFIG failed, ret = %d", -errno);
            goto write_err;
        ...
        int module_id, uint8_t *data, int num_bytes)
{
    ...
    if (error == 0) {
        ...
        if (tries >= 100) {
            ...
            ALOGE("%s:%d: Timed out waiting for status 0x100, got: 0x%04x",
                    __func__, __LINE__, cf_status);
            error = -EIO;
        }
    }
    ...
    if (error == 0) {
        if (rpc_status != STATUS_OK) {
            ...
            ALOGE("%s: rpc_status: %d", __func__, error);
        }
    }
    return error;
}
        ...
        uint32_t ready_bits, uint32_t ready_state)
{
    ...
    do {
        ...
        ALOGI("%s:%d: Waiting for 0x%04x, current state: 0x%04x",
                __func__, __LINE__, ready_state, value);
        tries++;
        usleep(1000);
...
static int tfa9887_force_coldboot(struct tfa9887_amp_t *amp)
{
    ...
    while (1) {
        if ((tries == 10) || (value & TFA9887_STATUS_ACS)) {
            break;
        }
...
static int tfa9887_otc(struct tfa9887_amp_t *amp)
{
    ...
    int tries = 50;
    uint16_t value;
    uint16_t mtp_ok = TFA9887_MTP_MTPOTC | TFA9887_MTP_MTPEX;

    rc = tfa9887_read_reg(amp, TFA9887_MTP, &value);
    if ((value & mtp_ok) == mtp_ok) {
    ...
    do {
        ...
        rc = tfa9887_read_reg(amp, TFA9887_STATUS, &value);
        tries--;
    } while ((value & TFA9887_STATUS_MTPB) && tries);

    if (tries == 0) {
        ALOGE("%s: OTC timed out\n", __func__);
    }

    rc = tfa9887_read_reg(amp, 0x80, &value);
    ALOGI("%s: Reset calibration done: 0x%04x\n", __func__, value);
...
static int tfa9887_hw_init(struct tfa9887_amp_t *amp, int sample_rate)
{
    ...
    if (error != 0) {
        ALOGE("%s: tfa9887 MTP still busy\n", __func__);
        goto priv_init_err;
    }
    ...
    if (error != 0) {
        ALOGE("%s: Unable to cold boot\n", __func__);
        goto priv_init_err;
    }
    error = tfa9887_set_sample_rate(amp, sample_rate);
    if (error != 0) {
        ALOGE("%s: Unable to set sample rate\n", __func__);
        goto priv_init_err;
    }
    error = tfa9887_select_channel(amp, channel);
    if (error != 0) {
        ALOGE("%s: Unable to select channel\n", __func__);
        goto priv_init_err;
    }
    error = tfa9887_select_input(amp, 2);
    if (error != 0) {
        ALOGE("%s: Unable to select input\n", __func__);
        goto priv_init_err;
    }
    error = tfa9887_set_volume(amp, 0.0);
    if (error != 0) {
        ALOGE("%s: Unable to set volume\n", __func__);
        goto priv_init_err;
    }
    error = tfa9887_hw_power(amp, true);
    if (error != 0) {
        ALOGE("%s: Unable to power up\n", __func__);
        goto priv_init_err;
    }
    usleep(5000);

    /* wait for ready */
    error = tfa9887_wait_ready(amp, TFA9887_STATUS_PLLS, TFA9887_STATUS_PLLS);
    if (error != 0) {
        ALOGE("%s: Failed to lock PLLs\n", __func__);
        goto priv_init_err;
    }

#ifdef WITH_COLDBOOT_PATCH
    /* This isn't needed most of the time */
    error = tfa9887_force_coldboot(amp);
    if (error != 0) {
        ALOGE("%s: Failed to force coldboot\n", __func__);
        goto priv_init_err;
    }
#endif
...
#ifdef WITH_OTC
    /* This isn't needed most of the time */
    error = tfa9887_otc(amp);
    if (error != 0) {
        ALOGE("%s: Failed to perform reset calibration\n", __func__);
        goto priv_init_err;
    }
#endif
    ...
    error = tfa9887_load_patch(amp, patch_buf, patch_size);
    if (error != 0) {
        ALOGE("%s: Unable to load patch data\n", __func__);
        goto priv_init_err;
    }

    error = tfa9887_load_dsp(amp, speaker_file);
    if (error != 0) {
        ALOGE("%s: Unable to load speaker data\n", __func__);
        goto priv_init_err;
    }
...
static int tfa9887_set_dsp_mode(struct tfa9887_amp_t *amp, uint32_t mode)
{
    ...
    if (amp->is_right) {
        ...
        ALOGV("%s: Setting right mode to %d\n", __func__, mode);
    } else {
    ...
    if (error != 0) {
        ALOGE("%s: Unable to set configured\n", __func__);
        goto set_dsp_err;
    }
    ...
    error = tfa9887_wait_ready(amp, TFA9887_STATUS_MTPB, 0);
    if (error != 0) {
        ALOGE("%s: Failed to become ready\n", __func__);
        goto set_dsp_err;
    }

    amp->is_dsp_mode_set = true;
    ALOGI("%s: Set %s DSP mode to %u\n",
...
int tfa9887_set_mode(audio_mode_t mode)
{
    ...
    for (i = 0; i < AMP_MAX; i++) {
        amp = &amps[i];
        if (!amp->is_dsp_mode_set) {
            ALOGI("%s: dsp mode not set, attempting re-init\n", __func__);
            rc = tfa9887_load_dsp(amp, amp->is_right ?
                "/system/etc/tfa/tfa9895.speaker" :
                "/system/etc/tfa/tfa9895_l.speaker");
