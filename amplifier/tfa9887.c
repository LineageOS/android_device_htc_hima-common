        ...
        uint16_t *val)
{
    ...
    if ((ret = ioctl(amp->fd, TFA9895_WRITE_CONFIG(sizeof(cmd_buf)),
                    &cmd_buf)) != 0) {
    ...
    memset(&cmd_buf, 0, sizeof(cmd_buf));
    cmd_buf.size = 2;
    if ((ret = ioctl(amp->fd, TFA9895_READ_CONFIG(sizeof(cmd_buf)),
                    &cmd_buf)) != 0) {
        ...
        const uint8_t *buf, int len)
{
    ...
    while (bytes_written < len) {
        memset(&cmd_buf, 0, sizeof(cmd_buf));
        cmd_buf.size = MIN(I2C_BUFFER_SIZE, len - bytes_written);
...
static int tfa9887_startup(struct tfa9887_amp_t *amp)
{
    ...
    tfa9887_write_reg(amp, TFA9887_SYSTEM_CONTROL, 0x0002);

    tfa9887_read_reg(amp, 0x8, &value);
    if (value & 0x400) {
        tfa9887_write_reg(amp, 0x8, (value & 0xFBFF));
        tfa9887_read_reg(amp, 0x8, &value);
        tfa9887_write_reg(amp, 0x8, value);
    }
...
static int tfa9887_lock(struct tfa9887_amp_t *amp, bool lock)
{
    ...
    rc = ioctl(amp->fd, TFA9895_KERNEL_LOCK(sizeof(cmd)), &cmd);
    if (rc) {
        rc = -errno;
...
static int tfa9887_enable_dsp(struct tfa9887_amp_t *amp, bool enable)
{
    ...
    rc = ioctl(amp->fd, TFA9895_ENABLE_DSP(sizeof(cmd)), &cmd);
    if (rc) {
        rc = -errno;
