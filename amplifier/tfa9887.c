        ...
        uint16_t val)
{
    ...
    cmd_buf.buffer[0] = 0;
    cmd_buf.buffer[1] = reg;
    cmd_buf.buffer[2] = (0xFF00 & val) >> 8;
    cmd_buf.buffer[3] = (0x00FF & val);
    if ((ret = ioctl(amp->fd, TFA9895_WRITE_CONFIG(cmd_buf),
                    &cmd_buf)) != 0) {
        ALOGE("ioctl TFA9895_WRITE_CONFIG failed, ret = %d", -errno);
        goto write_reg_err;
        ...
        const uint8_t *buf, int len)
{
    ...
    while (bytes_written < len) {
        memset(&cmd_buf, 0, sizeof(cmd_buf));
        cmd_buf.size = MIN(MAX_I2C_LENGTH, len - bytes_written);
        memcpy(cmd_buf.buffer, buf + bytes_written, cmd_buf.size);
        if ((ret = ioctl(amp->fd, TFA9895_WRITE_CONFIG(cmd_buf),
                        &cmd_buf)) != 0) {
            ALOGE("ioctl TFA9895_WRITE_CONFIG failed, ret = %d", -errno);
            goto write_err;
        }
        bytes_written += cmd_buf.size;
    }
...
static int tfa9887_force_coldboot(struct tfa9887_amp_t *amp)
{
    ...
    while ((tries < 10) && ((value & TFA9887_STATUS_ACS) == 0)) {
        rc = tfa9887_load_patch(amp, dsp_patch, 16);
        if (rc) {
            ALOGW("%s: Failed to load 16B patch, try: %d\n", __func__, tries);
        }
        tries++;
        usleep(5000);
    }
    ...
    if (tries == 10) {
        ALOGE("%s:%d: Timed out checking ACS\n", __func__, __LINE__);
        rc = -EBUSY;
    }
