        ...
        uint16_t ready_value, bool value_set, int max_tries, int interval)
{
    ...
    if (tries == max_tries) {
        rc = -EBUSY;
        ALOGE("%s: Timed out waiting for reg: 0x%02x, current: 0x%02x, expected: 0x%02x\n",
...
static int tfa9887_otc(struct tfa9887_amp_t *amp)
{
    ...
    skip_reset = (value & TFA9887_MTP_MTPOTC);
    if (!skip_reset) {
    ...
    rc = tfa9887_wait_reg(amp, TFA9887_STATUS, TFA9887_STATUS_MTP, false,
            10000, 10000);
    if (!skip_reset) {
...
static int tfa9887_reset_calibration(struct tfa9887_amp_t *amp)
{
    ...
    /* Wait until MTPB is not set */
    rc = tfa9887_wait_reg(amp, TFA9887_STATUS, TFA9887_MTPB, false, 10000, 50);
    if (rc) {
        ALOGE("%s: Timed out waiting for MTPB to clear: rc\n", __func__, rc);
        goto reset_cal_err;
    }
        ...
        float *re_25c)
{
    ...
    *re_25c = 0.0f;
    ...
    rc = tfa9887_get_data(amp, MODULE_SPEAKERBOOST, PARAM_GET_RE0, data, 3);
    if (rc) {
        ALOGE("%s: Failed to read SPEAKERBOOST param: %d\n", __func__, rc);
        goto get_cal_imp_err;
    }
    mem = 0;
    bytes2data(data, 3, &mem);
    *re_25c = ((float)mem) / (1 << (23 - SPKRBST_TEMP_EXP));
...
static int tfa9887_hw_init(struct tfa9887_amp_t *amp, int sample_rate)
{
    ...
    /* Wait until MTPB is not set */
    error = tfa9887_wait_reg(amp, TFA9887_STATUS, TFA9887_STATUS_MTPB, false,
            10, 1000);
    if (error != 0) {
    ...
    error = tfa9887_wait_reg(amp, TFA9887_STATUS, TFA9887_STATUS_PLLS, true,
            10, 1000);
    if (error != 0) {
        ...
        uint32_t *cal_value)
{
    ...
    if ((reg_value & TFA9887_MTP_MTPOTC)) {
        ...
        rc = tfa9887_wait_reg(amp, TFA9887_MTP, TFA9887_MTP_MTPEX, true, 30,
                50000);
        if (rc) {
            ALOGE("%s: Timed out waiting for MTP\n", __func__);
            *cal_value = 0;
            goto wait_cal_err;
        } else {
...
int tfa9887_open(void)
{
    ...
    for (i = 0; i < AMP_MAX; i++) {
        ...
        do {
...
#ifdef WITH_OTC
            /* Highly suspect this is only for MFG ROM */
            rc = tfa9887_otc(amp);
            if (rc) {
            ...
            if (rc) {
                ALOGE("%s: Failed waiting for calibration: %d\n", __func__, rc);
            }
            ...
            if (pass < 2) {
                /* TODO: reset calibration if out of range first/second time*/
                /* Recalibrate if impedance <= 5.0f || impedance >= 10.0f */
            } else if (pass == 2) {
                /* TODO: set calibration impedance to default of 7.0f third time*/
            }
            pass = 3;
        } while (pass < 3);
